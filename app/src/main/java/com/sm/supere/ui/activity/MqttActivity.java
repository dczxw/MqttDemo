...
public class MqttActivity extends AppCompatActivity implements IGetMessageCallBack, OnHandlerListener, View.OnClickListener {
        ...
        switch (view.getId()) {
            case R.id.btn_send:
                ...
                if (!TextUtils.isEmpty(topic) && !TextUtils.isEmpty(content)) {
                    ...
                    Toast.makeText(MqttActivity.this, "请输入发布主题或者信息", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.btn_sub:
                String topicSub = etSubTopic.getText().toString();
                if (TextUtils.isEmpty(topicSub)) {
                    Toast.makeText(MqttActivity.this, "请输入订阅主题", Toast.LENGTH_SHORT).show();
                    return;
                }
